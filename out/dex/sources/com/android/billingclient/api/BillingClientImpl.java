...
class BillingClientImpl extends BillingClient {
    ...
    private final class BillingServiceConnection implements ServiceConnection {
        ...
        public void onServiceConnected(ComponentName componentName, IBinder iBinder) {
            ...
            try {
                int iIsBillingSupported = BillingClientImpl.this.mService.isBillingSupported(6, packageName, BillingClient.SkuType.SUBS);
                if (iIsBillingSupported == 0) {
                    ...
                    iIsBillingSupported = BillingClientImpl.this.mService.isBillingSupported(5, packageName, BillingClient.SkuType.SUBS);
                    if (iIsBillingSupported == 0) {
                        BillingHelper.logVerbose(BillingClientImpl.TAG, "In-app billing API version 5 supported.");
                        BillingClientImpl.this.mSubscriptionUpdateSupported = true;
                        BillingClientImpl.this.mSubscriptionsSupported = true;
                    } else {
                        int iIsBillingSupported2 = BillingClientImpl.this.mService.isBillingSupported(3, packageName, BillingClient.SkuType.SUBS);
                        if (iIsBillingSupported2 == 0) {
                            ...
                            BillingClientImpl.this.mSubscriptionsSupported = true;
                            iIsBillingSupported = iIsBillingSupported2;
                        } else if (BillingClientImpl.this.mIABv6Supported) {
                            iIsBillingSupported = 0;
                        } else {
                            int iIsBillingSupported3 = BillingClientImpl.this.mService.isBillingSupported(3, packageName, BillingClient.SkuType.INAPP);
                            if (iIsBillingSupported3 == 0) {
                            ...
                            iIsBillingSupported = iIsBillingSupported3;
                        }
                    }
                }
                if (iIsBillingSupported == 0) {
                ...
                this.mListener.onBillingSetupFinished(iIsBillingSupported);
    ...
    public void consumeInternal(final String str, final ConsumeResponseListener consumeResponseListener) {
        try {
            ...
            final int iConsumePurchase = this.mService.consumePurchase(3, this.mApplicationContext.getPackageName(), str);
            if (iConsumePurchase == 0) {
                ...
                if (consumeResponseListener != null) {
                    postToUiThread(new Runnable() { // from class: com.android.billingclient.api.BillingClientImpl.5
                        ...
                        public void run() {
                            consumeResponseListener.onConsumeResponse(iConsumePurchase, str);
                        }
                    });
                }
            } else {
                BillingHelper.logWarn(TAG, "Error consuming purchase with token. Response code: " + iConsumePurchase);
        ...
        do {
            if (z) {
                try {
                    ...
                    purchaseHistory = this.mService.getPurchaseHistory(6, this.mApplicationContext.getPackageName(), str, string, null);
                } catch (RemoteException e) {
                ...
                purchaseHistory = this.mService.getPurchases(3, this.mApplicationContext.getPackageName(), str, string);
            }
            ...
            if (!purchaseHistory.containsKey(BillingHelper.RESPONSE_INAPP_ITEM_LIST) || !purchaseHistory.containsKey(BillingHelper.RESPONSE_INAPP_PURCHASE_DATA_LIST) || !purchaseHistory.containsKey(BillingHelper.RESPONSE_INAPP_SIGNATURE_LIST)) {
            ...
            string = purchaseHistory.getString(BillingHelper.INAPP_CONTINUATION_TOKEN);
            BillingHelper.logVerbose(TAG, "Continuation token: " + string);
    ...
    public int launchBillingFlow(Activity activity, BillingFlowParams billingFlowParams) {
        ...
        try {
            ...
            if (this.mIABv6Supported) {
                Bundle bundleConstructExtraParams = constructExtraParams(billingFlowParams);
                bundleConstructExtraParams.putString(LIBRARY_VERSION_KEY, "1.1");
                buyIntent = this.mService.getBuyIntentExtraParams(billingFlowParams.getVrPurchaseFlow() ? 7 : 6, this.mApplicationContext.getPackageName(), sku, skuType, null, bundleConstructExtraParams);
                str = TAG;
            } else {
                try {
                    if (z) {
                        ...
                        List<String> listAsList = Arrays.asList(billingFlowParams.getOldSku());
                        str2 = TAG;
                        buyIntent = iInAppBillingService.getBuyIntentToReplaceSkus(5, packageName, listAsList, sku, BillingClient.SkuType.SUBS, null);
                        str = str2;
                    } else {
    ...
    public void queryPurchaseHistoryAsync(final String str, final PurchaseHistoryResponseListener purchaseHistoryResponseListener) {
        if (isReady()) {
            executeAsync(new Runnable() { // from class: com.android.billingclient.api.BillingClientImpl.4
                @Override // java.lang.Runnable
                public void run() {
                    final Purchase.PurchasesResult purchasesResultQueryPurchasesInternal = BillingClientImpl.this.queryPurchasesInternal(str, true);
                    BillingClientImpl.this.postToUiThread(new Runnable() { // from class: com.android.billingclient.api.BillingClientImpl.4.1
                        @Override // java.lang.Runnable
                        public void run() {
                            purchaseHistoryResponseListener.onPurchaseHistoryResponse(purchasesResultQueryPurchasesInternal.getResponseCode(), purchasesResultQueryPurchasesInternal.getPurchasesList());
                        }
                    });
                }
            });
    ...
    public void querySkuDetailsAsync(SkuDetailsParams skuDetailsParams, final SkuDetailsResponseListener skuDetailsResponseListener) {
        ...
        if (TextUtils.isEmpty(skuType)) {
            ...
            executeAsync(new Runnable() { // from class: com.android.billingclient.api.BillingClientImpl.2
                @Override // java.lang.Runnable
                public void run() {
                    final SkuDetails.SkuDetailsResult skuDetailsResultQuerySkuDetailsInternal = BillingClientImpl.this.querySkuDetailsInternal(skuType, skusList);
                    BillingClientImpl.this.postToUiThread(new Runnable() { // from class: com.android.billingclient.api.BillingClientImpl.2.1
                        @Override // java.lang.Runnable
                        public void run() {
                            skuDetailsResponseListener.onSkuDetailsResponse(skuDetailsResultQuerySkuDetailsInternal.getResponseCode(), skuDetailsResultQuerySkuDetailsInternal.getSkuDetailsList());
                        }
                    });
                }
            });
