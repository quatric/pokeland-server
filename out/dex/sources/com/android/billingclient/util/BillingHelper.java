...
public final class BillingHelper {
    ...
    public static List<Purchase> extractPurchases(Bundle bundle) {
        ...
        if (stringArrayList == null || stringArrayList2 == null) {
            ...
            Purchase purchaseExtractPurchase = extractPurchase(bundle.getString(RESPONSE_INAPP_PURCHASE_DATA), bundle.getString(RESPONSE_INAPP_SIGNATURE));
            if (purchaseExtractPurchase == null) {
                logWarn(TAG, "Couldn't find single purchase data as well.");
                return null;
            }
            arrayList.add(purchaseExtractPurchase);
        } else {
            for (int i = 0; i < stringArrayList.size() && i < stringArrayList2.size(); i++) {
                Purchase purchaseExtractPurchase2 = extractPurchase(stringArrayList.get(i), stringArrayList2.get(i));
                if (purchaseExtractPurchase2 != null) {
                    arrayList.add(purchaseExtractPurchase2);
                }
            }
        }
        return arrayList;
